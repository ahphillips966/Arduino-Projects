...
bool isBtn1Down = false;
bool isBtn2Down = false;
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(2), turnPage, CHANGE);
  attachInterrupt(digitalPinToInterrupt(3), turnPage, CHANGE);

  Serial.println("Code has begun");
}
...
void turnPage() {
  ...
  if((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if(btn1==LOW && !isBtn1Down) {
      ...
      isBtn1Down = false;
      lastDebounceTime = millis();
      Serial.print("Reset Button1");
      }
  
    if(btn2==LOW && !isBtn2Down) {
      pageNum++;
      isBtn2Down = true;
      if(pageNum == 5){
        pageNum = 1;
